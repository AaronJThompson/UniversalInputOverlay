...
namespace Input_Overlay
{
    ...
    class Keyboard
    {
        ...
        private Key[] keys;
        public Keyboard(InputHook ih)
        {

            inputHook = ih;

            inputHook.onKeyDown += KeyDown;
            inputHook.onKeyUp += KeyUp;
            keys = new Key[254];
            // Top Row
            ...
            // 4th row
            keys[(int)Keys.ShiftKey] = new Key(0.55f, "Shift", (int)Keys.ShiftKey, new Point(5, 320), Properties.Resources.Shift, Properties.Resources.Shift_Pressed);
            ...
            //5th Row
            keys[(int)Keys.ControlKey] = new Key(0.55f, "Ctrl", (int)Keys.ControlKey, new Point(5, 420), Properties.Resources.Tab, Properties.Resources.Tab_Pressed);
            keys[(int)Keys.Menu] = new Key(0.55f, "Alt", (int)Keys.Menu, new Point(200, 420), Properties.Resources.Tab, Properties.Resources.Tab_Pressed);
        ...
        {
            Console.WriteLine("KeyDown: \t{0}", e.Data.KeyValue);
            if (keys[e.Data.KeyValue] != null)
            {
                keys[e.Data.KeyValue].Pressed = true;
            }
        }
        private void KeyUp(object sender, KeyEventArgs e)
        {
            Console.WriteLine("KeyUp: \t{0}", e.Data.KeyValue);
            if (keys[e.Data.KeyValue] != null)
                keys[e.Data.KeyValue].Pressed = false;
        }
        public void Stop()
        {
            inputHook.onKeyDown -= KeyDown;
